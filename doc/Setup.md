...
- `MailSync__BackoffPushUrl`: Optional push URL (e.g. an Uptime Kuma push monitor) used as a dead man's switch: pushed `status=up` every 15 minutes while no account is stuck in a failure run of repeated failed syncs, not pushed at all otherwise. Empty by default (no push). See [Synchronization.md](Synchronization.md#backoff-after-failed-syncs).
