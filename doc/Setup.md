...
- `MailSync__BackoffPushUrl`: Optional push URL (e.g. an Uptime Kuma push monitor) that receives `status=up` or `status=down` once a minute; `down` while any account is stuck in a failure run of repeated failed syncs. Empty by default (no push). See [Synchronization.md](Synchronization.md#backoff-after-failed-syncs).
